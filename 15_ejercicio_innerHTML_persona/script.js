let personas = {
    ...
    let nombreIngresado = prompt('Ingresa el nombre de la persona:');

    
    if (nombreIngresado in personas) {

        let persona = personas[nombreIngresado];

        let resultado = `
            <div style= "color: white; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 20px; background-color: ${persona.eyeColor};">
                <img src="${persona.picture}" alt="${persona.name}">
                <h1>${persona.name}</h1>
                <p>Age: ${persona.age}</p>
        ...
        let resultado = 'No se encontró información para esa persona.';
        document.body.innerHTML = resultado;
    }
